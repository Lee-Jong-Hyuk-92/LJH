...
def bayes_theorem():
    ...
    total_mails = 20 # 전체 메일 수
    spam_mails = 8  # 스팸 메일 수
    ham_mails = total_mails - spam_mails  # 정상 메일 수 (600개)
    
    keyword_in_spam = 5  # "확인"이 포함된 스팸 메일 개수
    keyword_in_ham = 2  # "확인"이 포함된 정상 메일 개수
    keyword_total = keyword_in_spam + keyword_in_ham  # "확인"이 등장한 전체 메일 수


    # 1. P(“스팸 메일”) 의 확률을 구하세요.
    p_spam = spam_mails / total_mails
    
    # 2. P(“확인” | “스팸 메일”) 의 확률을 구하세요.
    p_confirm_spam = keyword_in_spam / spam_mails
    
    # 3. P(“정상 메일”) 의 확률을 구하세요.
    p_ham = ham_mails / total_mails

    # 4. P(“확인” | "정상 메일" ) 의 확률을 구하세요.
    p_confirm_ham = keyword_in_ham / ham_mails
    
    # 5. P( "스팸 메일" | "확인" ) 의 확률을 구하세요.
    p_spam_confirm  = (keyword_in_spam) / (keyword_total)
    
    # 6. P( "정상 메일" | "확인" ) 의 확률을 구하세요.
    p_ham_confirm = (keyword_in_ham) / (keyword_total)
